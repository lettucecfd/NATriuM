...
namespace natrium {
    ...
    MixingLayer3D::MixingLayer3D(double viscosity, size_t refinementLevel,
 ...
 */
    boost::shared_ptr<Mesh<3> > MixingLayer3D::makeGrid(std::vector<unsigned int> repetitions) {
...
boost::shared_ptr<BoundaryCollection<3> > MixingLayer3D::makeBoundaries() {
    ...
    double MixingLayer3D::MeanVelocityProfile::value(const dealii::Point<3>& x,
		const unsigned int component) const{
...
double MixingLayer3D::IncompressibleU::value(const dealii::Point<3>& x,
		const unsigned int component) const{
    ...
    double
    MixingLayer3D::InitialTemperature::value(const dealii::Point<3> &x, const unsigned int component) const {
        double T = 1.0;
    ...
    double
    MixingLayer3D::InitialDensity::value(const dealii::Point<3> &x, const unsigned int component) const {
        double rho = 1.0;
        const double height = this->m_flow->getHeight();
        if (x(1) > (height / 2.0))
...
inline void MixingLayer3D::mean(vector<vector<double> > &matrix, double &avg){
...
inline void MixingLayer3D::angles(int nmodes, int &iseed, int &iy, vector<int> &iv, vector<double> &fi,
...
inline void MixingLayer3D::randf_1(int nmd, int alow, double ahigh, int idum, int &iy, vector<int> &iv,
