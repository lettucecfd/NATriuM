...
void getAnalyticSolution(double time, distributed_vector& analyticSolution1, distributed_vector& analyticSolution2,
		const vector<dealii::Point<2> >& supportPoints, const TaylorGreenVortex2D& tgVortex) {
	...
	for (size_t i = 0; i < supportPoints.size(); i++) {
		analyticSolution1(i) = tgVortex.analyticVelocity1(supportPoints.at(i), time);
		analyticSolution2(i) = tgVortex.analyticVelocity2(supportPoints.at(i), time);
	}
}
...
int main() {
	...
	configuration->setTimeStep(0.001*deltaX);
	configuration->setDQScaling(sqrt(3.*viscosity/configuration->getTimeStep()));
...
//configuration->getTimeStep()/3.;
	...
	std::ofstream maxNormOut(s.str().c_str());
	maxNormOut << "# t            max|u_numeric|            max|u_analytic|     max |rho-rho_0|" << endl;
	...
	dealii::DoFTools::map_dofs_to_support_points(solver.getAdvectionOperator()->getMapping(),
	...
	for (size_t i = 0; i < N; i++) {
		...
		getAnalyticSolution(configuration->getTimeStep()*i, analyticSolution1, analyticSolution2, supportPoints, *tgVortex);
		...
		maxNormOut << i*configuration->getTimeStep() << "  " << solver.getMaxVelocityNorm() << "  " << exp(-2*viscosity*i*configuration->getTimeStep()) << " " << solver.getMaxDensityDeviationFrom(1) << endl;
	}
