...
boost::shared_ptr<BoundaryCollection<2> > DiamondObstacle2D::makeBoundaries() {
	...
	boundaries->addBoundary(
			boost::make_shared<MinLeeBoundary<2> >(1, zeroVector));
	boundaries->addBoundary(
			boost::make_shared<MinLeeBoundary<2> >(2, boundary_density, boundary_velocity));
	boundaries->addBoundary(
			boost::make_shared<MinLeeBoundary<2> >(3, boundary_density, boundary_velocity));
	boundaries->addBoundary(
			boost::make_shared<MinLeeBoundary<2> >(4, zeroVector));
