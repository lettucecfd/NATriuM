/**
 * @file SmoothDensityPropagation.h
 * @short Description of a simple Periodic Flow (in rectangular domain).
 * @date 29.05.2013
 * @author Andreas Kraemer, Bonn-Rhein-Sieg University of Applied Sciences, Sankt Augustin
 */

#ifndef SmoothDensityPropagation_H_
#define SmoothDensityPropagation_H_

#include "deal.II/grid/tria.h"

#include "natrium/problemdescription/CompressibleBenchmark.h"
 ...
 */
class SmoothDensityPropagation: public CompressibleBenchmark<2> {
public:
	...
	private:
		SmoothDensityPropagation* m_flow;
	public:
		AnalyticVelocity(SmoothDensityPropagation* flow) :
				m_flow(flow) {
		}
	...
	private:
		SmoothDensityPropagation* m_flow;
	public:
		AnalyticDensity(SmoothDensityPropagation* flow) :
				m_flow(flow) {
		}
		virtual double value(const dealii::Point<2>& x, const unsigned int component=0) const;
	};

    class AnalyticTemperature: public dealii::Function<2> {
    private:
        SmoothDensityPropagation* m_flow;
    public:
        AnalyticTemperature(SmoothDensityPropagation* flow) :
                m_flow(flow) {
        }
        virtual double value(const dealii::Point<2>& x, const unsigned int component=0) const;
    };
	...
	SmoothDensityPropagation(double viscosity,
			size_t refinementLevel, double cs = 0.57735026919, bool init_rho_analytically = false, double L = 2*M_PI , double horizontal=1.0);

	/// destructor
	virtual ~SmoothDensityPropagation();
