...
int main() {
	...
	cout << "Starting NATriuM timeintegration analysis ..." << endl;
	...
	filename << getenv("NATRIUM_HOME")
			<< "/timeintegration-analysis/table_runtime.txt";
	...
	filename2 << getenv("NATRIUM_HOME")
			<< "/timeintegration-analysis/table_order.txt";
	std::ofstream orderFile(filename2.str().c_str());
	//orderFile << "# visc = " << viscosity << "; Ma = " << Ma << endl;
	orderFile
			<< "# 	CFL	 refinementlevel  i      t         max |u_analytic|  max |error_u|  max |error_rho|   ||error_u||_2   ||error_rho||_2"
			<< endl;
	size_t refinementLevel = 2;
	enum timeintegrationMethod {TEST };

 for (double CFL = 0.1; CFL < 1; CFL=CFL+0.1) {
		...
		shared_ptr<CouetteFlow2D> tgVortex = make_shared<
				CouetteFlow2D>(viscosity, 1/sqrt(3)*Ma,
						refinementLevel, 1.0, 0.0, 1);
