...
int main(int argc, char* argv[]) {
	...
	for (double gamma = 0.2;  gamma <= 1.0; gamma += 0.1) {
		shared_ptr<ProblemDescription<2> > sinusFlow = make_shared<
				SinusoidalShear2D>(viscosity, bottomVelocity, refinementLevel,
				L, averageHeight, amplitude);
		...
		dirName << getenv("NATRIUM_HOME") << "/sinus-shear/gamma_" << gamma;
