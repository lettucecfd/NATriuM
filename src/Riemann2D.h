 ...
 */
class Riemann2D: public ProblemDescription<2> {
public:
	...
	private:
		Riemann2D* m_flow;
	public:
		InitialVelocity(Riemann2D* flow) :
				m_flow(flow) {
		}
	...
	private:
		Riemann2D* m_flow;
	public:
		InitialTemperature(Riemann2D* flow) :
				m_flow(flow) {
		}
		virtual double value(const dealii::Point<2>& x, const unsigned int component=0) const;
	};
