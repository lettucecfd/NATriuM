    ...
    #define DEFINE_POSSIBLE_COLLISION(T_D, T_Q, STENCIL, COLLISION, EQUILIBRIUM, collision_type, equilibrium_type) \
		if ( (configuration.getStencil() == STENCIL) && \
            ...
            (configuration.getEquilibriumScheme() == EQUILIBRIUM) ) { \
				GeneralCollisionData<T_D, T_Q>                                                                                 \
                    genData(configuration, problemDescription, stencil.getScaling(),                \
                            viscosity, stencil, stencil.getSpeedOfSoundSquare(), delta_t); \
    ...
    #define DEFINE_POSSIBLE_COMPRESSIBLE_COLLISION(T_D, T_Q, STENCIL, COLLISION, EQUILIBRIUM, collision_type, equilibrium_type) \
		if ( (configuration.getStencil() == STENCIL) && \
				...
				typename collision_type<T_D, T_Q, equilibrium_type>::SpecificCollisionData\
					specData(genData);\
				CollisionOperator<T_D, T_Q, equilibrium_type, collision_type> collision_operator;\
                collision_operator.collideAll(f, g, densities, velocities, temperature, maskShockSensor, locally_owned_dofs,\
						inInitializationProcedure, genData, specData); \
				has_collided += 1; \
		}
 ...
 */
    ...
    inline void selectCollision<2>(SolverConfiguration& configuration,
        ...
        if (not has_collided) {
            throw CollisionException(
                    "Severe error: Collision model not implemented for incompressible solver yet -- cf. CollisionSelection.h");
        }
        if (has_collided > 1) {
            throw CollisionException(
    ...
    inline void selectCollision<3>(SolverConfiguration& configuration,
        ...
        DEFINE_POSSIBLE_COLLISION(3,27, Stencil_D3Q27, BGK_STANDARD, QUARTIC_EQUILIBRIUM, BGKCollision, QuarticEquilibrium);

        DEFINE_POSSIBLE_COLLISION(3,27, Stencil_D3V27, BGK_STANDARD, BGK_EQUILIBRIUM, BGKCollision, BGKEquilibrium);
        DEFINE_POSSIBLE_COLLISION(3,27, Stencil_D3V27, BGK_STANDARD, QUARTIC_EQUILIBRIUM, BGKCollision, QuarticEquilibrium);
        ...
        if (not has_collided) {
            throw CollisionException(
                    "Collision model not implemented for incompressible solver yet -- cf. CollisionSelection.h");
        }

    } // selectCollision 3D

    //// Templates for compressible solver (2nd distribution function g)
    ...
    inline void selectCollision<2>(SolverConfiguration& configuration,
        ...
        DEFINE_POSSIBLE_COMPRESSIBLE_COLLISION(2,777, Stencil_D2Q777, BGK_STANDARD, QUARTIC_EQUILIBRIUM, BGKCollision, QuarticEquilibrium);

        DEFINE_POSSIBLE_COMPRESSIBLE_COLLISION(2,25, Stencil_D2Q25H, BGK_REGULARIZED, BGK_EQUILIBRIUM, BGKCollision, BGKEquilibrium);

        if (not has_collided) {
            throw CollisionException(
                    "Severe error: Collision model not implemented for compressible solver yet -- cf. CollisionSelection.h");
        }
        if (has_collided > 1) {
            throw CollisionException(
                    "Severe error: More than one collision model was executed per time step. "
