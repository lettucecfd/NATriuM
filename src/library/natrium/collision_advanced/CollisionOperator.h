...
public:
	...
	void collideAll(DistributionFunctions& f, distributed_vector& densities,
			...
			typename T_collision<T_D, T_Q, T_equilibrium>::uniqueData& data) const {
		...
		for (it = locally_owned_dofs.begin(); it != end; it++) {
			...
			calculateVelocity<T_D, T_Q>(fLocal, params.velocity, params.scaling,
					params.density, params); // TODO velocities for other stencils

			//Write the local density to the global velocity matrix
			if (not inInitializationProcedure) {

				for (size_t j = 0; j < T_D; ++j) {
					velocities.at(j)(i) = params.velocity[j] * params.scaling;
				}
			}
			...
			collisionScheme.relax(fLocal, params, data);
