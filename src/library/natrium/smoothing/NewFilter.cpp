...
template<size_t dim>
NewFilter<dim>::NewFilter(double alpha, double s,
		const dealii::Quadrature<dim>& quadrature,
		const dealii::FiniteElement<dim>& fe) :
		m_alpha(alpha), m_s(s), m_p(fe.degree), m_quadrature(quadrature), m_sourceFE(
...
template<size_t dim>
void natrium::NewFilter<dim>::makeProjectionMatrices(
...
template<>
inline double natrium::NewFilter<1>::evaluateLegendreND(size_t i,
		const dealii::Point<1>& x) {
	assert(i < m_legendre1D.size());
	return m_legendre1D.at(i).value(x(0));
}
template<>
inline double natrium::NewFilter<2>::evaluateLegendreND(size_t i,
...
template<>
inline double natrium::NewFilter<3>::evaluateLegendreND(size_t i,
...
template<>
inline void NewFilter<1>::makeDegreeVectors(size_t p) {
...
template<>
inline void NewFilter<2>::makeDegreeVectors(size_t p) {
...
template<>
inline void NewFilter<3>::makeDegreeVectors(size_t p) {
...
template<size_t dim>
void NewFilter<dim>::applyFilter(
...
template class NewFilter<1> ;
template class NewFilter<2> ;
template class NewFilter<3> ;
