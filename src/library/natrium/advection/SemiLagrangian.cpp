...
// Define update flags (which values have to be known at each cell, face, neighbor face)
	...
	const dealii::UpdateFlags faceUpdateFlags = update_quadrature_points | update_JxW_values
...
template<size_t dim>
void SemiLagrangian<dim>::assembleLocalMassMatrix(
...
template<size_t dim>
void SemiLagrangian<dim>::assembleLocalDerivativeMatrices(
		const dealii::FEValues<dim>& feValues, size_t dofs_per_cell,
...
template<size_t dim>
void SemiLagrangian<dim>::assembleAndDistributeLocalFaceMatrices(size_t alpha,
...
template<> void SemiLagrangian<2>::calculateAndDistributeLocalStiffnessMatrix(
		size_t alpha,
...
template<> void SemiLagrangian<3>::calculateAndDistributeLocalStiffnessMatrix(
		size_t alpha,
		const vector<dealii::FullMatrix<double> >& derivativeMatrices,
		dealii::FullMatrix<double> &systemMatrix,
		const vector<double>& inverseLocalMassMatrix,
...
template<size_t dim>
void SemiLagrangian<dim>::assembleAndDistributeInternalFace(size_t alpha,
		typename dealii::DoFHandler<dim>::active_cell_iterator& cell,
		size_t faceNumber,
		typename dealii::DoFHandler<dim>::cell_iterator& neighborCell,
