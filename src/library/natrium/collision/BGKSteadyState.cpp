...
BGKSteadyState::BGKSteadyState(double relaxationParameter, double dt,
	...
	assert (preconditioning_parameter <= 1);
} /// constructor

/// destructor
BGKSteadyState::~BGKSteadyState() {
} /// destructor

double BGKSteadyState::getEquilibriumDistribution(size_t i,
	...
	if (0 == i) {
		return prefactor * (1 + 1./m_gamma * uSquareTerm);
	}
	double mixedTerm = Math::scalar_product(u, getStencil()->getDirection(i))
			/ getStencil()->getSpeedOfSoundSquare();
	return prefactor * (1 + mixedTerm * (1 + 0.5 / m_gamma * (mixedTerm)) + 1./m_gamma * uSquareTerm);
...
void BGKSteadyState::collideAll(DistributionFunctions& f,
	...
	if (Stencil_D2Q9 != getStencil()->getStencilType()) {
		...
		for (it = locally_owned_dofs.begin(); it != end; it++){
			...
			uSquareTerm = -scalar_product / cs2;
			// direction 0
			weighting = 4. / 9. * densities(i);
			feq.at(0) = weighting * (1 + one_by_2gamma * uSquareTerm);
			// directions 1-4
			weighting = 1. / 9. * densities(i);
			mixedTerm = prefactor * (velocities.at(0)(i));
			feq.at(1) = weighting
					* (1 + mixedTerm * (1 + one_by_2gamma * mixedTerm) + one_by_2gamma * uSquareTerm);
			feq.at(3) = weighting
					* (1 - mixedTerm * (1 - one_by_2gamma * mixedTerm) + one_by_2gamma * uSquareTerm);
