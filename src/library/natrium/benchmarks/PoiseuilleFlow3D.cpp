...
boost::shared_ptr<Mesh<3> > PoiseuilleFlow3D::makeGrid(double height, double width,
	...
	MPI_COMM_WORLD);
	dealii::GridGenerator::hyper_rectangle(*rect, dealii::Point<3>(0, 0, 0.0),
			dealii::Point<3>(length, width, height), false);

	// Assign boundary indicators to the faces of the "parent cell"
	Mesh<3>::active_cell_iterator cell = rect->begin_active();
	...
	cell->face(2)->set_all_boundary_ids(4);  // bottom
	cell->face(3)->set_all_boundary_ids(5);  // top
