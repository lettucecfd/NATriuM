 ...
 */
class CouetteFlowGrad2D: public Benchmark<2> {
public:
	...
	private:
		CouetteFlowGrad2D* m_benchmark;
	public:
		AnalyticVelocity(CouetteFlowGrad2D* couette) :
				m_benchmark(couette) {
		}
	...
	/// constructor
	CouetteFlowGrad2D(double viscosity, double topPlateVelocity,
			size_t refinementLevel, double L = 1.0, double startTime = 0.0,
			bool isUnstructured = false);

	/// destructor
	virtual ~CouetteFlowGrad2D();
