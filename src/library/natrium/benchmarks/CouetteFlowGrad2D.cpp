/**
 * @file CouetteFlowGrad2D.cpp
 * @short 
 * @date 29.05.2013
 * @author Andreas Kraemer, Bonn-Rhein-Sieg University of Applied Sciences, Sankt Augustin
 */

#include "CouetteFlowGrad2D.h"
...
CouetteFlowGrad2D::CouetteFlowGrad2D(double viscosity, double topPlateVelocity,
...
CouetteFlowGrad2D::~CouetteFlowGrad2D() {
}

void CouetteFlowGrad2D::refine(Mesh<2>& mesh){
	// refine grid
	mesh.refine_global(m_refinementLevel);

}

void CouetteFlowGrad2D::transform(Mesh<2>& mesh){
...
boost::shared_ptr<Mesh<2> > CouetteFlowGrad2D::makeGrid(double L) {
...
boost::shared_ptr<BoundaryCollection<2> > CouetteFlowGrad2D::makeBoundaries(
...
double CouetteFlowGrad2D::AnalyticVelocity::value(const dealii::Point<2>& x,
