...
public:
	CompressibleCFDSolver(boost::shared_ptr<SolverConfiguration> configuration,
        ...
        m_temperature.reinit(this->getAdvectionOperator()->getLocallyOwnedDofs(),
                             this->getAdvectionOperator()->getLocallyRelevantDofs(), MPI_COMM_WORLD);
        m_tmpTemperature.reinit(this->getAdvectionOperator()->getLocallyOwnedDofs(), MPI_COMM_WORLD);
        ...
        if(!checkpointExists) {
            ...
            getWriteableMember(writeable_temperature, m_temperature, this->getAdvectionOperator()->getLocallyOwnedDofs());
            this->applyInitialTemperatures(writeable_temperature, this->getSupportPoints());
            applyWriteableMember(writeable_temperature, m_temperature);
        ...
        if (checkpointExists)
        {
            ...
            getWriteableMember(writeable_T, m_temperature, this->m_advectionOperator->getLocallyOwnedDofs());
            ...
            for (; it != end; it++) {
                ...
                for (size_t i = 0; i < this->m_stencil->getQ(); i++) {
                    ...
                    for (size_t a = 0; a < dim; a++) {
                        sum += (this->getStencil()->getDirection(i)[a] - this->m_velocity.at(a)(i))
                             * (this->getStencil()->getDirection(i)[a] - this->m_velocity.at(a)(i));
                    }
            ...
            applyWriteableMember(writeable_T, m_temperature);
        }
	}
...
//        CheckpointStatus checkpoint_status;
        ...
        if (0 != restart_i) {
            checkpoint = boost::make_shared<Checkpoint<dim>>(restart_i, checkpoint_dir, true);
        ...
        if (checkpoint) {
            ...
            dealii::IndexSet::ElementIterator it(this->m_advectionOperator->getLocallyOwnedDofs().begin());
            dealii::IndexSet::ElementIterator end(this->m_advectionOperator->getLocallyOwnedDofs().end());
...
//	distributed_block_vector& g = m_g.getFStream();
	const distributed_sparse_block_matrix& systemMatrix = this->getAdvectionOperator()->getSystemMatrix();
	...
	if (SEMI_LAGRANGIAN == this->getConfiguration()->getAdvectionScheme()) {
		...
		if (this->getConfiguration()->isVmultLimiter()) {
			TimerOutput::Scope timer_section(Timing::getTimer(), "Limiter");
			VmultLimiter::apply(systemMatrix, m_g.getFStream(), g_tmp.getFStream());
		}
	}
	else
    {
...
// start timer
	...
	if (this->m_configuration->isFiltering()) {
		if (this->m_i % this->m_configuration->getFilterInterval() == 0) {
			for (size_t i = 0; i < this->m_stencil->getQ(); i++) {
				this->m_filter->applyFilter(*(this->m_advectionOperator)->getDoFHandler(), this->m_f.at(i));
				this->m_filter->applyFilter(*(this->m_advectionOperator)->getDoFHandler(), m_g.at(i));
			}
		}
		this->m_f.updateGhosted();
		m_g.updateGhosted();
...
void calculateTemperature() {
    ...
    getWriteableMember(writeable_u, this->getVelocity(), this->getAdvectionOperator()->getLocallyOwnedDofs());
    getWriteableMember(writeable_rho, this->getDensity(), this->getAdvectionOperator()->getLocallyOwnedDofs());
    getWriteableMember(writeable_T, this->getTemperature(), this->getAdvectionOperator()->getLocallyOwnedDofs());
 ...
 *
 */
inline distributed_vector& getWriteableMember(distributed_vector& writeable, const distributed_vector& member, const dealii::IndexSet& locally_owned){
...
inline std::vector<distributed_vector>& getWriteableMember(std::vector<distributed_vector>& writeable, const std::vector<distributed_vector>& member, const dealii::IndexSet& locally_owned){
 ...
 */
inline void applyWriteableMember(const distributed_vector& writeable, distributed_vector& member){
    TimerOutput::Scope timer_section(Timing::getTimer(), "Copy vectors");
    member = writeable;
}
inline void applyWriteableMember(const vector<distributed_vector>& writeable, vector<distributed_vector>& member){
    TimerOutput::Scope timer_section(Timing::getTimer(), "Copy vectors");
...
void applyShockSensor() {
    ...
    getWriteableMember(writeable_Mask, this->m_maskShockSensor, this->getAdvectionOperator()->getLocallyOwnedDofs());
    ...
    applyWriteableMember(writeable_Mask, this->m_maskShockSensor);
}

void smoothDensities(const map<dealii::types::global_dof_index, dealii::Point<dim> >& supportPoints)  {
    array<double,25> feq;
    array<double,2> u;
    // get Function instance
...
// start timer
    ...
    try {
        ...
        vector<distributed_vector> writeable_u;
        distributed_vector writeable_rho;
        distributed_vector writeable_T;
        distributed_vector writeable_mSS;
        getWriteableMember(writeable_u, this->m_velocity, this->getAdvectionOperator()->getLocallyOwnedDofs());
        getWriteableMember(writeable_rho, this->m_density, this->getAdvectionOperator()->getLocallyOwnedDofs());
        getWriteableMember(writeable_T, this->m_temperature, this->getAdvectionOperator()->getLocallyOwnedDofs());
        getWriteableMember(writeable_mSS, this->m_maskShockSensor, this->getAdvectionOperator()->getLocallyOwnedDofs());
        ...
        selectCollision(*(this->m_configuration), *(this->m_problemDescription), this->m_f, m_g, writeable_rho,
                        writeable_u, writeable_T, writeable_mSS, this->m_advectionOperator->getLocallyOwnedDofs(),
                        this->m_problemDescription->getViscosity(), delta_t, *(this->getStencil()), false);
        ...
        applyWriteableMember(writeable_rho, this->m_density);
        applyWriteableMember(writeable_u, this->m_velocity);
        applyWriteableMember(writeable_mSS, this->m_maskShockSensor);
...
inline void calcQuarticEquilibrium(vector<double>& feq, size_t T_Q, double density, array<double,dim> velocity,
    ...
    double uu_term = 0.0;
    for (size_t j = 0; j < dim; j++) {
        uu_term += -(velocity[j] * velocity[j]) / (2.0 * cs2);
    }
...
//for all degrees of freedom on current processor
    const dealii::IndexSet& locally_owned_dofs = this->m_advectionOperator->getLocallyOwnedDofs();
    dealii::IndexSet::ElementIterator it(locally_owned_dofs.begin());
    dealii::IndexSet::ElementIterator end(locally_owned_dofs.end());
...
//                const bool inInitializationProcedure = true;
            ...
            while (loopCount < this->m_configuration->getIterativeInitializationNumberOfIterations()) {
                distributed_vector rho;
                vector<distributed_vector> u;
                distributed_vector T;
                distributed_vector mSS;
                // get writeable copies of rho and u
                getWriteableMember(oldDensities, this->m_density, locally_owned_dofs);
                getWriteableMember(rho, this->m_density, locally_owned_dofs);
                getWriteableMember(mSS, this->m_maskShockSensor, locally_owned_dofs);
                getWriteableMember(u, this->m_velocity, locally_owned_dofs);
                getWriteableMember(T, m_temperature, locally_owned_dofs);
                try {
                    this->stream();
                ...
                try {
                    ...
                    selectCollision(*(this->m_configuration), *(this->m_problemDescription), this->m_f, m_g, rho, u, T, mSS,
        ...
        default: {
            throw CFDSolverException("Error in CFDSolver::InitializeDistributions. "
                                     "A part of the code was reached, which should never be reached. "
                                     "With ITERATIVE you enforce it for compressible flows, however");
        }
    }
