...
// output: vector fields as .vtu files
        ...
        void collide() {
            ...
            try {
                ...
                selectCollision(*(this->m_configuration), *(this->m_problemDescription), this->m_f, m_g, writeable_rho,
                                writeable_u, writeable_T, writeable_mSS,
        ...
        inline void
        calcQuarticEquilibrium(std::vector<double> &feq, size_t T_Q, double density, std::array<double, dim> velocity,
                               double temperature, double cs2, std::vector<std::array<double, dim>> e,
            ...
            for (size_t i = 0; i < T_Q; i++) {
                ...
                for (size_t alp = 0; alp < dim; alp++) {
                    for (size_t bet = 0; bet < dim; bet++) {
                        feq[i] += density * weight[i] / (2.0 * cs2) *
                                  ((temperature - 1) * eye[alp][bet] * e[i][alp] * e[i][bet] -
                        ...
                        for (size_t gam = 0; gam < dim; gam++) {
                            ...
                            for (size_t det = 0; det < dim; det++) {
                                ...
                                feq[i] += weight[i] * density / (24. * cs2 * cs2 * cs2 * cs2) *
                                          (power4 - cs2 * power2 + cs2 * cs2 * power0) *
        ...
        void initializeDistributions() {
            ...
            double descaled_cs2 = this->m_stencil->getSpeedOfSoundSquare() /
                                  (this->getStencil()->getScaling() * this->getStencil()->getScaling());
                ...
                case COMPRESSIBLE_ITERATIVE: {
                    ...
                    while (loopCount
                        ...
                        try {
                            ...
                            selectCollision(*(this->m_configuration), *(this->m_problemDescription), this->m_f, m_g,
                                            rho, u, T, mask,
                                            this->m_advectionOperator->getLocallyOwnedDofs(),
                                            this->m_problemDescription->getViscosity(), delta_t, *(this->getStencil()),
                                            true);
